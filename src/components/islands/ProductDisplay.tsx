...
const ProductDisplay = ({ products, selectedCategory, allProducts }: ProductDisplayProps) => {
    // Usar allProducts si está disponible, sino usar products
    const productsForCategories = allProducts || products

    const categories = Array.from(new Set(productsForCategories.map((product) => product.category))).map((category) => ({
        name: category,
    ...
    return (
        <div className="w-full flex flex-col items-center">
            <FilterBar categories={categories} selectedCategory={selectedCategory ?? null} />
            <div className="w-11/12 h-fit flex flex-col tablet:flex-row flex-wrap items-center justify-center gap-4">
                {filteredProducts.map((product) => (
                    <ProductCard key={product.id} product={product} selectedCategory={selectedCategory} />
                ))}
            </div>
        </div>
    )
